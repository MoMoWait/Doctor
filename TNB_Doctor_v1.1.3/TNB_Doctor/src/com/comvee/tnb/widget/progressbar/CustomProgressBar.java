...
public class CustomProgressBar extends ProgressBar {
	 ...
	 * implements the {@linkplain CustomProgressBar.Formatter} interface.
	 * 
	 * @param formatter
	 */
	...
	protected synchronized void onDraw(Canvas canvas) {
		...
		if (m_indicator != null) {
			...
			if (getProgress() > 8)
				dx = dx - getIndicatorWidth() * 3 / 4 - m_offset + getPaddingLeft();
			else
				dx = 0 - getIndicatorWidth() / 4 - m_offset + getPaddingLeft();
			...
			canvas.drawText(m_formatter != null ? m_formatter.getText(getProgress()) : Math.round(getScale(getProgress()) * 100.0f) + "",
	...
	private void updateProgressBar() {
		...
		if (progressDrawable != null && progressDrawable instanceof LayerDrawable) {
			...
			int progress = getProgress()<10&&getProgress()!=0?9:getProgress();
			final float scale = getScale(progress);

			// get the progress bar and update it's size
			Drawable progressBar = d.findDrawableByLayerId(R.id.progress);

			final int width = d.getBounds().right - d.getBounds().left-getPaddingLeft();

			if (progressBar != null) {
				Rect progressBarBounds = progressBar.getBounds();
				progressBarBounds.right = progressBarBounds.left + (int) (scale != 0 ? width * scale : 0)+getPaddingLeft();
				if(progress == 0){
